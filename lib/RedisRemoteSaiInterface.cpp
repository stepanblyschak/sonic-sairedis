        ...
        _Out_ sai_status_t *object_statuses)
{
    ...
    for (size_t idx = 0; idx < values.size(); idx++)
    {
        ...
        SaiAttributeList list(objectType, entries, false);
        ...
        // TODO (stepanb): How about preserving the logic w.r.t to status code? Check the method comment.

        transfer_attributes(objectType, attr_count[idx], list.get_attr_list(), attr_list[idx], false);
    }

    m_recorder->recordBulkGenericGetResponse(status, objectType, object_count, attr_count, attr_list, object_statuses);
        ...
        _Inout_ sai_status_t *object_statuses)
{
    ...
    for (size_t idx = 0; idx < serialized_object_ids.size(); idx++)
    {
        ...
        */
        ...
        swss::FieldValueTuple fvt(serialized_object_ids[idx] , str_attr);
    ...
    m_recorder->recordBulkGenericGet(key, entries);
    ...
    auto status = waitForBulkGetResponse(object_type, object_count, attr_count, attr_list, object_statuses);
